...
impl From<u64> for Square {
    fn from(v: u64) -> Square {
        if v >= 64 {
            panic!("Don't try to name a square off the edge of the board.");
        } else {
            Square(v as usize)
        }
    }
...
mod tests {
    ...
    fn square_from_index() {
        assert_eq!(Square::try_from(0usize), Ok(A1));
        assert_eq!(Square::try_from(63usize), Ok(H8));
        assert_eq!(Square::try_from(64usize), Err(()));
    }
