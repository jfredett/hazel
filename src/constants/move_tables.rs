...
lazy_static! {
    ...
    pub static ref NOMINAL_ROOK_ATTACKS : [Bitboard; 64] = {
        ...
        for rank in 0..8 {
            for file in 0..8 {
                let idx = rank * 8 + file;
                out[idx] = (RANK_MASKS[rank] | FILE_MASKS[file]) & !Bitboard::from(1 << idx);
            }
        }
        return out
    };
...
mod test {
    ...
    fn nominal_bishop_attacks_calculate_correctly() {
        ...
        let expected : Bitboard = (*A1_H8_DIAG | *A7_G1_DIAG) & !bitboard!("d4");
        // d4 is 0o33
    ...
    fn nominal_rook_attacks_calculate_correctly_in_middle_of_board() {
        ...
        let expected : Bitboard = (*D_FILE | *RANK_4) & !bitboard!("d4");
        // d4 is 0o33
    ...
    fn nominal_rook_attacks_calculate_correctly_on_corner_of_board() {
        // stick a rook on d4, it should see...
        let expected : Bitboard = (*A_FILE | *RANK_1) & !bitboard!("a1");
        // d4 is 0o33
