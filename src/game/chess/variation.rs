...
impl Variation {
    ...
    pub fn new_game(&mut self) -> &mut Self {
        self.record(Action::Setup(FEN::start_position()));
        self
    }
    ...
    pub fn setup(&mut self, fen: FEN) -> &mut Self {
        self.record(Action::Setup(fen.clone()));
        self
    }
    ...
    pub(crate) fn get_cursor(&self) -> Cursor<Action> {
        self.log.raw_cursor()
    }
    ...
    */
    ...
    fn record(&mut self, action: Action) -> &mut Self {
        if self.halted { return self; }
