...
mod tests {
    ...
    mod pext {
        ...
        fn pext_pexts_correctly(bb1_in: u64, mask_in: u64) -> bool {
            let bb = Bitboard::from(bb1_in);
            let mask = Bitboard::from(mask_in);


            unsafe {
                core::arch::x86_64::_pext_u64(bb1_in, mask_in) == bb.pext(mask)
            }
        }
    }
    ...
    mod first_index {
        ...
        #[quickcheck]
        fn first_index_is_equilvalent_to_all_set_indices_at_position_zero_for_nonempty_bitboards(bb: Bitboard) -> bool {
            if bb.is_empty() { return true }
