...
impl Ply {
    ...
    pub fn piece_at(&self, file: File, rank: usize, piece: Piece, color: Color) -> bool {
        if !(1..=8).contains(&rank) {
            panic!("Invalid position {:?}{:?}", file, rank);
        }
    ...
    pub fn piece_at_index(&self, idx: usize) -> Option<(Color, Piece)> {
        for color in COLORS {
            if self.rooks_for(color).is_index_set(idx) {
                return Some((color, Piece::Rook));
            }
            if self.bishops_for(color).is_index_set(idx) {
                return Some((color, Piece::Bishop));
            }
            if self.knights_for(color).is_index_set(idx) {
                return Some((color, Piece::Knight));
            }
            if self.king_for(color).is_index_set(idx) {
                return Some((color, Piece::King));
            }
            if self.queens_for(color).is_index_set(idx) {
                return Some((color, Piece::Queen));
            }
            if self.pawns_for(color).is_index_set(idx) {
                return Some((color, Piece::Pawn));
            }
        }
        None
    }
    ...
    pub fn friendly_piece_at_index(&self, idx: usize) -> Option<Piece> {
        if self.rooks_for(self.current_player()).is_index_set(idx) {
            return Some(Piece::Rook);
        }
        if self.bishops_for(self.current_player()).is_index_set(idx) {
            return Some(Piece::Bishop);
        }
        if self.knights_for(self.current_player()).is_index_set(idx) {
            return Some(Piece::Knight);
        }
        if self.king_for(self.current_player()).is_index_set(idx) {
            return Some(Piece::King);
        }
        if self.queens_for(self.current_player()).is_index_set(idx) {
            return Some(Piece::Queen);
        }
        if self.pawns_for(self.current_player()).is_index_set(idx) {
    ...
    pub fn enemy_piece_at_index(&self, idx: usize) -> Option<Piece> {
        if self.rooks_for(self.other_player()).is_index_set(idx) {
            return Some(Piece::Rook);
        }
        if self.bishops_for(self.other_player()).is_index_set(idx) {
            return Some(Piece::Bishop);
        }
        if self.knights_for(self.other_player()).is_index_set(idx) {
            return Some(Piece::Knight);
        }
        if self.king_for(self.other_player()).is_index_set(idx) {
            return Some(Piece::King);
        }
        if self.queens_for(self.other_player()).is_index_set(idx) {
            return Some(Piece::Queen);
        }
        if self.pawns_for(self.other_player()).is_index_set(idx) {
            return Some(Piece::Pawn);
        }
        None
    }
    ...
    pub fn can_castle_long(&self) -> bool {
        match self.current_player() {
            Color::WHITE => {
                self.meta.white_castle_long
                    && (self.occupancy() & bitboard!("b1", "c1", "d1")).is_empty()
            }
            Color::BLACK => {
                self.meta.black_castle_long
                    && (self.occupancy() & bitboard!("b8", "c8", "d8")).is_empty()
            }
        }
    }
    ...
    pub fn can_castle_short(&self) -> bool {
        match self.current_player() {
            Color::WHITE => {
                self.meta.white_castle_short
                    && (self.occupancy() & bitboard!("f1", "g1")).is_empty()
            }
            Color::BLACK => {
                self.meta.black_castle_short
                    && (self.occupancy() & bitboard!("f8", "g8")).is_empty()
            }
        }
    }
}
