...
impl Ply {
    ...
    pub fn queen_attack_board_for(&self, color: Color) -> Bitboard {
        self.slider_attacks_for(Piece::Queen, color)
    }
    /// Calculates all squares attacked by all rooks of the given color.
    pub fn rook_attack_board_for(&self, color: Color) -> Bitboard {
        self.slider_attacks_for(Piece::Rook, color)
    }
    /// Calculates all squares attacked by all bishops of the given color.
    pub fn bishop_attack_board_for(&self, color: Color) -> Bitboard {
        self.slider_attacks_for(Piece::Bishop, color)
    }
...
mod tests {
    ...
    mod attacked_squares_for {
        ...
        fn expected_attacks() -> Bitboard {
            bitboard!(
                "b8", "a7", "c7", "d7", "h7", "a6", "b6", "g6", "b5", "d5", "e5", "f5", "g5", "b4",
                "c4", "d4", "e4", "f4", "h4", "a3", "b3", "c3", "h3", "b2", "e2", "h2", "b1", "c1",
                "d1", "e1", "f1", "h1"
            )
        }

        fn expected_pawn_attacks() -> Bitboard {
            bitboard!("d4", "f4", "h4", "b3", "d3", "e3", "f3", "g3", "h3")
        }

        #[test]
        fn correctly_determines_all_attacked_squares() {
            assert_eq!(
                test_position().attacked_squares_for(Color::WHITE),
                expected_attacks()
            );
        }
        ...
        fn correctly_determines_pawn_attacked_squares() {
            assert_eq!(
                test_position().pawn_attack_board_for(Color::WHITE),
                expected_pawn_attacks()
            );
        }
    }
