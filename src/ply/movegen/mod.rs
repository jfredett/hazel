...
impl Ply {
    ...
    fn my_pawns(&self) -> Bitboard { self.pawns_for(self.current_player()) }
    fn my_knights(&self) -> Bitboard { self.knights_for(self.current_player()) }
    fn my_bishops(&self) -> Bitboard { self.bishops_for(self.current_player()) }
    fn my_rooks(&self) -> Bitboard { self.rooks_for(self.current_player()) }
    fn my_queens(&self) -> Bitboard { self.queens_for(self.current_player()) }
    fn my_king(&self) -> Bitboard { self.king_for(self.current_player()) }
    ...
    fn pawn_advances(&self) -> MoveSet {
        ...
        for target_idx in moves {
            ...
            out.add_move(Piece::Pawn, self.enemy_pawn_direction().index_shift(target_idx), target_idx);
        }
